...
import { useGetCollectionByIdQuery, useGetUserByIdQuery } from '../../store/queries/users';
...
function Collection() {
  ...
  if(isSuccess) {
    ...
    data.collections.forEach((value: ICollection) => {
        const { data: dataVocab, isSuccess: isSuccessVocabs } = useGetCollectionByIdQuery(value.id)
        if(isSuccessVocabs) {
    ...
    return (
      <div className="p-[20px]">
        ...
        {myCollection.length === 0 ? (
          <div className='mt-10'>
            <p className='mb-4 text-error font-medium text-[20px]'>You don't have any vocabularies. Let's make your Collection</p>
